...
| `MAX_PAGE_OUTPUT_CHARS` | `16384` | `<PAGE>` 기준 페이지별 decoded 출력 문자 hard limit (single에도 동일 적용) |
| `MAX_PAGE_OUTPUT_TOKENS` | `6144` | 페이지별 생성 토큰 hard limit (fast decode는 최대 한 block만큼 정지 지연) |
| `PAGE_SEPARATOR` | `\n\n---\n\n` | 병합 시 페이지 구분자 |
...
| `OPENAI_BASE_URL` | (없음) | 번역 프로바이더 base URL (버전 경로 포함, 예 `https://api.openai.com/v1`). 미설정 시 번역 기능만 비활성(503) |
