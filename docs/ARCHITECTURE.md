...
- **실패 격리**: 렌더에서 한 페이지가 깨지면 흰색 페이지로 대체하고 계속하며, ocr 청크가
  실패하면 캐시 해제 후 1회 재시도 → 그래도 실패하면 해당 페이지들을 실패 플레이스홀더로
  보정하고 잡을 done으로 마감한다(내역은 meta.json `warnings`, 전 청크/전 페이지 실패 시에만
  error, 취소는 그대로 전파).
